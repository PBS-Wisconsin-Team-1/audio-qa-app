...
def detect_clipping(audio, sr, threshold=0.98, min_clip_duration_ms=10):
    frame_length = int((min_clip_duration_ms / 1000) * sr)
    ...
    intervals = rms_frame_intervals_seconds(len(rms), sr, frame_length, hop, duration_s=duration_s)

    # Detect frames above threshold
    clipped_frames = rms > threshold

    # Group consecutive frames into regions using start/end of covered intervals
    regions = []
    start_time = None
    end_time = None
    for i, is_clipped in enumerate(clipped_frames):
        if is_clipped:
            if start_time is None:
                start_time = intervals[i][0]
            end_time = intervals[i][1]
        elif not is_clipped and start_time is not None:
